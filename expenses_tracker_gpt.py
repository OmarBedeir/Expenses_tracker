...
class ExpenseTracker:
    ...
    def setup_gui(self):
        ...
        style = ttk.Style()
        style.configure("Treeview", font=('Arial', 12), rowheight=25)  # font and row height
        ...
        columns = ["Original Amount", "Category", "Date", "Payment Method", "Original Currency", "Converted Amount (USD)"]
        self.tree = ttk.Treeview(self.root, columns=columns, show="headings")
        for col in columns:
            self.tree.heading(col, text=col)
            # Tag configuration for the "Converted Amount (USD)" column
            if col == "Converted Amount (USD)":
                self.tree.tag_configure(col, background='#FFC107')  #  color 
        ...
        #  'total_row' with a background color
    ...
    def add_expense(self):
        ...
        # Update table with the converted amount
        item_id = self.tree.insert("", "end", values=(amount, category, date_str, payment_method, original_currency, converted_amount))
